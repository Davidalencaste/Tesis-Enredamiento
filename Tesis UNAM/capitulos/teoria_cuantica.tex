...
\end{equation}
donde $\left(\cdot,\cdot\right)$ representa el producto interno en $\mathcal{H}$ \cite{Cohen}. Por lo tanto, se puede definir el producto interno en términos de kets y bras \cite{Messiah} como
\begin{equation}
...
El espacio de estados entonces debe cumplir con todas las propiedades del producto interno, además,  toda sucesión de Cauchy debe converger a un elemento del espacio, es decir, debe ser un \textit{espacio completo}. Además, la base del espacio debe ser orto-normal numerable, a estos espacios se les conoce como \textit{espacios separables}. Estas restricciones son las que definen a un \textit{espacio de Hilbert} \cite{Messiah}.
...
Un espacio de Hilbert ($H$), es un espacio lineal con producto interno definido, completo y separable \cite{Messiah}.

Su producto interno, definido como: $\left(\cdot,\cdot \right):  H\times H\to \mathbb{C}$ satisface las siguientes condiciones:
\begin{itemize}
    \item $\left(x,y \right)=\overline{\left(y,x \right)}$
    \item $\left(\alpha x + \beta y,z \right)=\alpha\left(x,y \right)+\beta\left(y,z \right)$
    \item $\left(x,x \right)\geq 0$
    \item Si $\left(x,x \right)=0$, entonces $x=0$
\end{itemize}
...
\begin{comment}
    Usando el formalismo de elementos de Hilbert, siendo $\ket{\psi}$ un elemento de este y $\hat{A}$ una variable dinámica, se puede obtener el valor esperado de $\hat{A}$ como sigue:
\begin{equation}
    \expval{A} = \expval{A}{\psi}
\end{equation}
Hay que notar que las fases globales en los estados no afectan el valor esperado de una variable dinámica, como se muestra a continuación:\\
Sea $\ket{\Psi}=e^{i\theta}\ket{\psi}$ 
\begin{equation}
    \expval{A}{\Psi}=e^{-i\theta}e^{i\theta}\expval{A}{\psi}=\expval{A}{\psi}
\end{equation}
Es decir, la física del sistema no cambia por una fase global. Esto nos permitiría trabajar con un conjunto de estados distintos que son físicamente equivalentes. Este conjunto es definido como \textit{espacio de rayo} \cite{Sakurai2011}.\\ 
\end{comment}
Sea $\ket{\psi}\in \mathcal{H}$, se define su espacio de rayo como:
\begin{equation}
    \textit{Ray Space}\equiv\{\lambda\ket{\psi} \; |\; \lambda\in \mathbb{C} \}.
\end{equation}
La motivación de la definición de este conjunto es que, en realidad, toda la información del estado físico se encuentra contenida de un elemento del espacio de estados, sin importar su fase global. En otras palabras, $\lambda\ket{\psi}$ contiene la misma información física que $\ket{\psi}$ para todo $\lambda\in\mathbb{C}$ \cite{Sakurai2011}.\\

Esto nos permitiría trabajar con un conjunto de estados distintos que son físicamente equivalentes.

\subsection{Espacio de Operadores de Densidad}
En el caso de sistemas con múltiples estados cuánticos (no superposición), es decir, estos dividen su ensamble en diferentes secciones donde cada sección es representada por un estado $\ket{\psi_i}$ distinto.
Un ejemplo clásico es el de un solido con diferentes dominios magnéticos, cada dominio posee una dirección particular de espín. ¿Como se puede describir su función de estado? Usando el formalismo de vector de Hilbert la descripción de este estado no es posible pues no existe una manera de dividir en sub-estados en este sentido.\\

Este tipo de ensambles motivo el formalismo de operador de Densidad, en donde es posible hacer esta subdivisión de estados.\\

Sea un ensamble con N $\ket{\psi_i}$ sub-estados, se define el \textit{operador de densidad} como:
\begin{equation}
