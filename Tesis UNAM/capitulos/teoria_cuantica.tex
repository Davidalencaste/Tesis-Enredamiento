...
\begin{itemize}
    ...
    \item $\ip{\psi}\geq 0$
    \item Si $\ip{\psi}=0$, entonces $\ket{\psi}=\bf{0}$
\end{itemize}
\begin{comment}
...
\end{equation}
Esto permite definir las clases de equivalencia de la siguiente forma:
\begin{equation}
    [\psi] := \{\lambda \ket{\psi} \mid \lambda \in \C_0\}.
\end{equation}

Al fijar el módulo del factor complejo igual a uno, $\varrho = 1$, se elimina la
...
global, $\ket{\psi} \sim \e^{i\theta}\ket{\psi}$, se obtiene el espacio
proyectivo de Hilbert, definido como el conjunto de todos los rayos del espacio
de Hilbert,
\begin{equation}
    \Proj(\H_0)
    :=
    \{(\ket{\psi}\in \H_0\,|\, \norm{\psi}=1)/(\ket{\psi}\sim \e^{i\theta}\ket{\psi})\}.
\end{equation}

El espacio $\Proj(\H_0)$ es conocido en física como el \emph{espacio de rayos},
mientras que en matemáticas corresponde a un espacio proyectivo de Hilbert.
En el caso de sistemas de dimensión finita, este espacio se denomina
\emph{espacio proyectivo complejo} y se denota por $\C \textbf{P}(\H_0)$.

Por lo tanto, el espacio proyectivo complejo proporciona una descripción del
...
\textit{Ejemplo Qubit:}
El espacio de estados en este caso es $\C\textbf{P} \simeq S^2$, que es comúnmente llamada la \textbf{esfera de Bloch}.
