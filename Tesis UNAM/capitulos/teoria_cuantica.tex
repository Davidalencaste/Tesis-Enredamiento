...
\begin{equation}
    \text{\textbf{Estado cuántico}} \equiv \ket{\psi}.
\end{equation}
...
\end{equation}
donde $\qty(\cdot,\cdot)$ representa el producto interno en $\mathcal{H}$ \cite{Cohen}. Así, el producto interno en términos de kets y bras \cite{Messiah}:
\begin{equation}
...
Es importante hacer notar que \textit{no es lo mismo ensamble promedio y valor esperado}; sin embargo, ambos conceptos están relacionados, ya que la ecuación \eqref{def:ensamble_promedio} notamos que $\qty[\bm{A}]$ depende del valor esperado $\expval{A}$.\\
...
\begin{enumerate}
    ...
    \item La superposición de operadores de estado es también un operador de estado; es decir, sean $\rhon_1,\rhon_2$ operadores de estado y $c_1,c_2\in\C$ con $\abs{c_1}^2+\abs{c_2}^2=1$. Entonces,
    \begin{equation}
        \rhon = \abs{c_1}^2\rhon_1 + \abs{c_2}^2\rhon_2,
...
Ahora, consideremos un ensamble general $\rhon=\sum_i P_i \op{\psi_i}$; de esta manera,
\begin{equation}
    \rhon^2=\sum_{i,j}P_iP_j\ket{\psi_i}\ip{\psi_i}{\psi_j}\bra{\psi_j},
    \label{eq:rho2_general}
\end{equation}
calculando la traza de \eqref{eq:rho2_general}:
\begin{align}
    \Tr(\rhon^2)&=
    \sum_{i,j}P_iP_j\ip{\psi_i}{\psi_j}\Tr(\op{\psi_i}{\psi_j})\notag\\
    &=\sum_{i,j}P_iP_j\ip{\psi_i}{\psi_j}\ip{\psi_j}{\psi_i}\notag\\
    &=\sum_{i,j}P_iP_j\abs{\ip{\psi_i}{\psi_j}}^2\notag\\
    &<\sum_{i,j}P_iP_j=\sum_iP_i\sum_jP_j=1,
    \label{eq:calculo_traza}
\end{align}
además en el caso con sub-estados todos ortogonales; es decir $\ip{\psi_i}{\psi_j}=0$, se concluye que $0\leq\Tr(\rhon^2)$, y por \eqref{eq:calculo_traza} se obtiene la siguiente propiedad para \textit{ensambles no puros}:
...
\begin{equation}
    \rhon = \abs{c_1}^2\rhon_1+\abs{c_2}^2\rhon_2 = \abs{c_1}^2\op{\phi_1}+\abs{c_2}^2\op{\phi_2},
...
\end{align}
de esta ultima expresión despejamos $\op{\psi_i}{\psi_j}$ y obtenemos,
\begin{equation}
...
\begin{align}
    ...
    &\approx \Id,
\end{align}
donde de igual forma asumimos que $\bm{H}$ es hermitico.\\ Ahora usando la propiedad de composición temporal \eqref{eq:composicion_evolcuion_temporal} podemos expresar el operador $\bm{U}$ como sigue:
\begin{align}
    \bm{U}(t_0+\dd{t},t_0)&=
