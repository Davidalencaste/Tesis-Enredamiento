...
Veamos como se comporta esta medida para estados no puros; consideremos un ensamble general de la forma $\rhon=\sum_i P_i \op{\psi_i}$ , por lo que
\begin{equation}
...
Para resolver esta pregunta podríamos considerar el estado superpuesto:
...
\begin{equation}
    \rhon = \abs{c_1}^2\rhon_1+\abs{c_2}^2\rhon_2 = \abs{c_1}^2\op{\psi_1}+\abs{c_2}^2\op{\psi_2},
...
\begin{equation}
    ...
    +\qty(c_1c_2^*\;\rhon_1\rhon_0\rhon_2+c_1^*c_2\;\rhon_2\rhon_0\rhon_1)\qty[\Tr(\rhon_1\rhon_o\rhon_2\rhon_0)]^{-1/2},
    \label{def:Ley_de_composicion}
\end{equation}
note de esta última expresión; el rol del estado fiducial $\ket{\psi_0}$, es el describir la ley de composición en unicamente términos de $\rhon_1$ y $\rhon_2$.\\ Por tanto, notamos la diferencia de la suma convencional de operadores $\eqref{eq:superposicion_operadores2}$ es el termino de la derecha, que es conocido como \textit{termino de purificación}, debido a que $\eqref{def:Ley_de_composicion}$ termina siendo un estado puro de la superposición de dos estados  \cite{MankoComposition}.

La expresión \eqref{def:Ley_de_composicion} se puede reescribir de la siguiente manera,
\begin{equation}
    \rhon=\sum_{i,j=1}^2c_ic_j^*\frac{\rhon_i\rhon_0\rhon_j}{\qty[\Tr(\rhon_i\rhon_0\rhon_j\rhon_0)]^{1/2}},
\end{equation}
que puede ser generalizada inmediatamente para $n$ estados de la siguiente manera \cite{MankoComposition} :
\begin{equation}
    \rhon = \sum_{i,j=1}^nc_ic_j^*\frac{\rhon_i\rhon_0\rhon_j}{\qty[\Tr(\rhon_i\rhon_0\rhon_j\rhon_0)]^{1/2}}.
\end{equation}
